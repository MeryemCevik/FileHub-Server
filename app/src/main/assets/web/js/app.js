...
async function confirmRename() {
    ...
    try {
        const url = `${CONFIG.endpoints.rename}?old=${encodeURIComponent(renameTarget)}&new=${encodeURIComponent(newName)}`;
        const res = await fetch(url, { method: 'POST' });

        if (res.ok) {
            closeRenameModal();
            selectedPaths.clear();
            updateToolbarState(); // Forcer la mise à jour de l'UI
            loadFiles(currentPath, false);
        } else {
            const errorText = await res.text();
            alert("Erreur serveur : " + (errorText || "Impossible de renommer l'élément."));
        }
    } catch (e) {
